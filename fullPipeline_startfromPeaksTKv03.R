## fullPipelineScript_startfromPeaksTKv03.R 
## Version 3.0 Wednesday, December 10th, 2014
## Created By: Nathan Cormier, Tyler Kolisnik, and Mark Bieda.

## DESCRIPTION:
# This is a program to change run the full pipeline script starting from the peak file and assumes that you have a mapped reads file.
...
## See these examples below when using different species or genome assembly, all annotation packages can be found at the bioconductor website: http://www.bioconductor.org/packages/3.0/data/annotation/.
...
## The input file, MACSpeakFile, outputDirectory and keplerBinLocation must be complete file paths, with no trailing /
## Note: This workflow assumes you do have the sequence read data, it uses a .bed file, AND a .sam or .bam file.
...
outputDirectory <- "/home/tylerk/massTestResults/fullScriptStartPeaks"  # The directory where all output files will be located
